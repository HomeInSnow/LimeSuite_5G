...
int DPDTest::readdata_PCIe_5GRadio()
{

    int status = 0;
    const int bytesToRead = 24 * 8192 * 2;
    ...
    else
    {
        ...
        {
            int samplesReceived = bytesReceived / 24; //one sample, 4 bytes * 3 channels
            ...
            unsigned offset = bytesReceived / 6;
            for (int i = 0; i < samplesReceived; i++)
            {
            ...
            std::cout << "[INFO] "
                      << "DPD received samples " << bytesReceived << "/" << bytesToRead << std::endl;
    }
...
int DPDTest::readdata_PCIe_5GRadio2()
{
    ...
    else
    {
        ...
        if (bytesReceived == bytesToRead)
        {
            int samplesReceived = bytesReceived / 24;
            int16_t value;
            for (int i = 0; i < samplesReceived; i++)
            {
                value = (buffer[4 * i]) & 0xFF;
...
void DPDTest::OnbtnStartClick(wxCommandEvent &event)
{
    ...
    std::cout << "[INFO] "
              << "DPD:" << temps1 << ",N=" << QADPD_N << ",M=" << QADPD_M << ",ND=" << QADPD_ND << ",GAIN=" << QADPD_GAIN << std::endl;
...
{
    mWorkerThread = std::thread(&DPDTest::readdata_PCIe_5GRadio, this);
}
...
int DPDTest::SPI_write(lms_device_t *lmsControl, uint16_t address, uint16_t data)
{
    ...
    //uint16_t maddress = 64;  // LimeSDR_QPCIe
    uint16_t maddress = 416;  // PCIe_5GRadio
...
int DPDTest::SelectSource(int CHA)
{
    ...
    // for PCIe_5GRadio board this is obsolate
    
    int ret = 0;
    
    //uint16_t maddress = 0x0041; // LimeSDR_QPCIe
    uint16_t maddress = 0x01A1; // PCIe_5GRadio
