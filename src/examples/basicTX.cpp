...
int main(int argc, char** argv)
{
    ...
    const double sample_rate = 30.72e6;    //sample rate to 5 MHz
    const double tone_freq = 5e6; //tone frequency
    ...
    if (LMS_EnableChannel(device, LMS_CH_TX, 0, true)!=0)
        error();
    //Enable TX channel,Channels are numbered starting at 0
    if (LMS_EnableChannel(device, LMS_CH_TX, 2, true)!=0)
        error();
    ...
    lms_stream_t tx_stream2;                 //stream structure
    tx_stream2.channel = 2;                  //channel number
    tx_stream2.fifoSize = 1024*1024;          //fifo size in samples
    tx_stream2.throughputVsLatency = 0.5;    //0 min latency, 1 max throughput
    tx_stream2.dataFmt = lms_stream_t::LMS_FMT_F32; //floating point samples
    tx_stream2.isTx = true;                  //TX channel
    LMS_SetupStream(device, &tx_stream2);
    ...
    for (int i = 0; i <buffer_size; i++) {      //generate TX tone
        ...
        tx_buffer[2*i] = 0;//cos(w);
        tx_buffer[2*i+1] = 0;//sin(w);
    }   
    ...
    while (chrono::high_resolution_clock::now() - t1 < chrono::seconds(100)) //run for 10 seconds
    {
        ...
        //Transmit samples
       /* ret = LMS_SendStream(&tx_stream2, tx_buffer, send_cnt, nullptr, 1000);
        if (ret != send_cnt)
            cout << "error tx2: samples sent: " << ret << "/" << send_cnt << endl;*/
/*
