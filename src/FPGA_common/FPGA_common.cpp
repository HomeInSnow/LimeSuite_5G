...
namespace lime
{
    ...
    int FPGA::SetPllClock(int clockIndex, int nSteps, bool waitLock, uint16_t &reg23val)
    {
        ...
        values.push_back(reg23val & ~PLLCFG_START);
        addrs.push_back(0x0024);
        values.push_back(abs(nSteps));         //CNT_PHASE
        ...
        addrs.push_back(0x0023);
        values.push_back(reg23val | PHCFG_START);
...
*/
    int FPGA::SetPllFrequency(const uint8_t pllIndex, const double inputFreq, FPGA_PLL_clock *clocks, const uint8_t clockCount)
    {
        ...
        vector<uint32_t> addrs;
        vector<uint32_t> values;
        addrs.push_back(0x0025);
        values.push_back(reg25 | 0x80);
        addrs.push_back(0x0023);
        ...
        /**/ t1 = chrono::high_resolution_clock::now();
        if (boardType == LMS_DEV_LIMESDR_QPCIE && clocks->findPhase == false)
            do //wait for reset to activate
            {
        ...
        values.push_back(M_N_odd_byp);
        int nlow = N / 2;
        int nhigh = nlow + N % 2;
        addrs.push_back(0x002A);
        values.push_back(nhigh << 8 | nlow); //N_high_cnt, N_low_cnt
        addrs.push_back(0x002B);
        values.push_back(mhigh << 8 | mlow);
        ...
        /**/ t1 = chrono::high_resolution_clock::now();
        if (boardType == LMS_DEV_LIMESDR_QPCIE)
            do //wait for config to activate
            {
                statusReg = ReadRegister(busyAddr);
                done = statusReg & 0x1;
                errorCode = (statusReg >> 7) & 0xFF;
    ...
    int FPGA::SetDirectClocking(int clockIndex)
    {
        ...
        addres.push_back(0x0005);
        values.push_back(drct_clk_ctrl_0005 | (1 << clockIndex));
...
*/
    int FPGA::SetInterfaceFreq(double txRate_Hz, double rxRate_Hz, int channel)
    {
        ...
        std::cerr << "FPGA_common2" << std::endl;
        ...
        if (rxRate_Hz >= 5e6 && txRate_Hz >= 5e6)
        {
            ...
            if ((vals[0] == 0xE && vals[1] > 0x20E) || (vals[0] == 0xF && vals[1] > 0x206) || (vals[0] == 0x10 && vals[1] > 0x102) || vals[0] == 0x17 || vals[0] == 0x19)
                phaseSearch = true;
        }
        ...
        lime::FPGA::FPGA_PLL_clock clocks[3]; // B.J.
        ...
        */
        ...
        for (int i = 0; i < 10; i++) //attempt phase search 10 times
        {
            ...
            clocks[1].index = 1;
            clocks[1].phaseShift_deg = txPhC1 + txPhC2 * txRate_Hz / 2.0;
            clocks[1].outFrequency = txRate_Hz / 2.0;
        ...
        if (!phaseSearchSuccess)
        {
            ...
            //sprintf(str, "%3.1f", txRate_Hz / 1e6);
            //lime::error(str);
            //status = -1;
            /*
        clocks[0].index = 0;
        ...
        */
            ...
            clocks[2].index = 2;
            clocks[2].phaseShift_deg = 0 ; //txPhC1 + txPhC2 * txRate_Hz;
            clocks[2].outFrequency = txRate_Hz;
